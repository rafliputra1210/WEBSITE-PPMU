...
@section('title', 'Edit Rekening PPDB')
...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <div>
        <h4 class="fw-bold mb-0">Edit Rekening PPDB</h4>
        <p class="text-muted mb-0 small">Ubah data rekening bank untuk pembayaran pendaftaran</p>
    </div>
</div>

<div class="card shadow-sm" style="max-width:600px;">
    <div class="card-body p-4">
        <form method="POST" action="{{ route('admin.pembayaran-ppdb.update', $pembayaranPpdb) }}">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <select name="entitas" class="form-select @error('entitas') is-invalid @enderror" required>
                    ...
                    <option value="pesantren" {{ old('entitas', $pembayaranPpdb->entitas) == 'pesantren' ? 'selected' : '' }}>🕌 Pesantren</option>
                    <option value="madrasah" {{ old('entitas', $pembayaranPpdb->entitas) == 'madrasah' ? 'selected' : '' }}>🏫 Madrasah</option>
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nama_bank" class="form-control @error('nama_bank') is-invalid @enderror"
                       placeholder="Contoh: BSI, BRI, BNI, Mandiri" value="{{ old('nama_bank', $pembayaranPpdb->nama_bank) }}" required>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="no_rekening" class="form-control @error('no_rekening') is-invalid @enderror"
                       placeholder="Contoh: 7172 8399 01" value="{{ old('no_rekening', $pembayaranPpdb->no_rekening) }}" required>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="atas_nama" class="form-control @error('atas_nama') is-invalid @enderror"
                       placeholder="Nama pemilik rekening" value="{{ old('atas_nama', $pembayaranPpdb->atas_nama) }}" required>
            ...
            <div class="mb-4">
                ...
                <textarea name="keterangan" class="form-control @error('keterangan') is-invalid @enderror" rows="3"
                          placeholder="Contoh: Biaya pendaftaran Rp 150.000 — Transfer ke rekening ini lalu kirim bukti ke admin.">{{ old('keterangan', $pembayaranPpdb->keterangan) }}</textarea>
